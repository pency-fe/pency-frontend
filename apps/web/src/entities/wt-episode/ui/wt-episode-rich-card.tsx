...
type WtEpisodeRichCardFnProps = {
  ...
  Menu: ComponentType<{ data: WtEpisodeRichCardFnProps["data"] } & ComponentProps<typeof Menux>>;
  hideGenre?: boolean;
};

const WtEpisodeRichCardFn = forwardRef<HTMLDivElement, WtEpisodeRichCardFnProps>(
  ({ data, hideGenre = false, Menu }, ref) => {
    ...
    return (
      <RichCard
        ref={ref}
        slots={{
          overlayElement: <RichCard.OverlayAnchor href={`/@${data.channel.url}/webtoon/episode/${data.id}`} />,
          thumbnail: (
            <RichCard.Thumbnail
              slots={{
...
export const WtEpisodeRichCard = Object.assign(WtEpisodeRichCardFn, {
  Loading: LoadingFn,
});
